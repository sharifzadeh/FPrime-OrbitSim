...
namespace Svc {
    ...
    )
  {
    ...
    // Serialize for IPC
    _status = msg.serializeFrom(static_cast<FwEnumStoreType>(CMD_CLOSEFILE));
    FW_ASSERT (
      _status == Fw::FW_SERIALIZE_OK,
      static_cast<FwAssertArgType>(_status)
    );
    ...
    ) const
  {
    ...
    if (this->m_logOut_OutputPort[0].isConnected()) {
      ...
      _status = file.serializeTo(_logBuff, FW_MIN(FW_LOG_STRING_MAX_SIZE, 240));
      FW_ASSERT(
        _status == Fw::FW_SERIALIZE_OK,
        static_cast<FwAssertArgType>(_status)
      );
    ...
    ) const
  {
    ...
    if (this->m_logOut_OutputPort[0].isConnected()) {
      ...
      _status = validationFile.serializeTo(_logBuff, FW_MIN(FW_LOG_STRING_MAX_SIZE, 240));
      FW_ASSERT(
        _status == Fw::FW_SERIALIZE_OK,
        static_cast<FwAssertArgType>(_status)
      );
  ...
  void ComLoggerComponentBase ::
    log_DIAGNOSTIC_FileClosed(const Fw::StringBase& file) const
  {
    // Get the time
    Fw::Time _logTime;
    if (this->m_timeCaller_OutputPort[0].isConnected()) {
    ...
    doDispatch()
  {
    ...
    if (_msgType == COMLOGGER_COMPONENT_EXIT) {
    ...
    switch (_msgType) {
      ...
      case CMD_CLOSEFILE: {
        ...
        // Call handler function
        this->CloseFile_cmdHandler(_opCode, _cmdSeq);
